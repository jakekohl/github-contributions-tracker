...
async function getUserContributions(
  username,
  startDate = new Date('2001-01-01T00:00:00Z').toISOString(),
  endDate,
) {
  console.debug(
    `Fetching user contributions for ${username} from ${startDate} (${typeof startDate}) to ${endDate} (${typeof endDate})`,
  )
  if (!username || !startDate || !endDate) {
    ...
    console.debug(
      `Start date: ${startDate} (${typeof startDate}), End date: ${endDate} (${typeof endDate})`,
    )
    return {
  ...
  try {
    ...
    while (currentStartDate <= endDate) {
      ...
      currentStartDate = new Date(
        currentStartDateObj.getUTCFullYear() + 1,
        0,
        1,
        0,
        0,
